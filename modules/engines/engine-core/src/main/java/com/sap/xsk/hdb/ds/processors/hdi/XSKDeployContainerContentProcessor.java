...
public class XSKDeployContainerContentProcessor extends XSKHDIAbstractProcessor {
  ...
  public final void execute(
      Connection connection, String container, String[] deployPaths, String[] undeployPaths)
      throws SQLException {
    executeUpdate(
        connection,
    ...
    for (String next : deployPaths) {
      executeUpdate(
          connection, "INSERT INTO #DEPLOY_PATHS (PATH) VALUES ('" + next.substring(1) + "');");
    }
    executeUpdate(
        connection,
    ...
    for (String next : undeployPaths) {
      executeUpdate(
          connection, "INSERT INTO #UNDEPLOY_PATHS (PATH) VALUES ('" + next.substring(1) + "');");
    }
    executeUpdate(
        connection,
