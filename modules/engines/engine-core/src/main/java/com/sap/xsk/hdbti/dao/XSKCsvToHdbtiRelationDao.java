...
public class XSKCsvToHdbtiRelationDao implements IXSKCsvToHdbtiRelationService {
  ...
  @Inject
  private PersistenceManager<XSKTableImportToCsvRelation>
      xskTableImportToCsvRelationPersistenceManager;

  @Inject private DataSource dataSource;

  @Override
  public void persistNewCsvAndHdbtiRelations(
      XSKTableImportArtifact tableImportArtifact, Connection connection) {
    for (XSKTableImportToCsvRelation relation :
        tableImportArtifact.getTableImportToCsvRelations()) {
  ...
  public void deleteCsvAndHdbtiRelations(String hdbtiFileName, Connection connection) {
    String sql =
        String.format(
            "DELETE FROM \"XSK_TABLE_IMPORT_TO_CSV\" WHERE \"HDBTI_LOCATION\"='%s'",
            XSKUtils.convertToFullPath(hdbtiFileName));
  ...
  public List<XSKTableImportToCsvRelation> getAllHdbtiToCsvRelations() {
    ...
    try (Connection connection = dataSource.getConnection()) {
      listOfcsvToHdbtiRelations =
          xskTableImportToCsvRelationPersistenceManager.findAll(
              connection, XSKTableImportToCsvRelation.class);
    } catch (SQLException e) {
  ...
  @Override
  public boolean hasCsvChanged(
      XSKTableImportToCsvRelation tableImportToCsvRelation, String csvContent) {
  ...
  public List<XSKTableImportToCsvRelation> getAffectedHdbtiToCsvRelations(String csvFilePath) {
    ...
    return relations.stream()
        .filter(relation -> relation.getCsv().equals(XSKUtils.convertToFullPath(csvFilePath)))
  ...
  public String convertToActualFileName(String fileNamePath) {
    ...
    return "/registry/public/"
        + fileNamePath.substring(0, fileNamePath.indexOf(':')).replaceAll("\\.", "/")
        + "/"
        + fileName;
  }
}
