...
public abstract class HDBCalculationViewAbstractProcessor {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(HDBCalculationViewAbstractProcessor.class);
  ...
  protected void grantPrivilegesToContainerSchema(
      Connection connection, String container, String user) throws SQLException {
    executeUpdate(
        connection,
        "CREATE LOCAL TEMPORARY COLUMN TABLE #PRIVILEGES LIKE _SYS_DI.TT_SCHEMA_PRIVILEGES;");
    executeUpdate(
        connection,
        "INSERT INTO #PRIVILEGES(PRIVILEGE_NAME, PRINCIPAL_SCHEMA_NAME, PRINCIPAL_NAME) VALUES ('SELECT', '', '"
            + user
            + "');");
    executeQuery(
        connection,
        "CALL "
            + container
            + "#DI.GRANT_CONTAINER_SCHEMA_PRIVILEGES(#PRIVILEGES, _SYS_DI.T_NO_PARAMETERS, ?, ?, ?);");
  ...
  protected void grantPrivilegesToContainerAPI(
      Connection connection, String group, String container, String user) throws SQLException {
    executeUpdate(
        connection,
  ...
  protected void writeContentInContainer(
    ...
    executeUpdate(
        connection,
        "INSERT INTO #PATHS (PATH, CONTENT) VALUES ('.hdiconfig', '{ \"plugin_version\" : \"12.1.0\", \"file_suffixes\" : { \"hdbcalculationview\" : { \"plugin_name\" : \"com.sap.hana.di.calculationview\" }}}')");
    for (String path : folders) {
      executeUpdate(
          connection, "INSERT INTO #PATHS (PATH, CONTENT) VALUES ('" + path + "', NULL);");
    }
    for (XSKDataStructureHDBCalculationViewModel next : hdbCalculationViews) {
      executeUpdate(
          connection,
          "INSERT INTO #PATHS (PATH, CONTENT) VALUES ('"
              + next.getLocation().substring(1)
              + "', '"
              + next.getContent()
              + "');");
    }
  ...
  protected void createContainer(Connection connection, String group, String container)
      throws SQLException {
    executeQuery(
        connection,
        "CALL _SYS_DI#"
  ...
  protected void grantPrivilegesToContainerGroup(Connection connection, String group, String user)
      throws SQLException {
    executeUpdate(
        connection,
        "CREATE LOCAL TEMPORARY COLUMN TABLE #PRIVILEGES LIKE _SYS_DI.TT_API_PRIVILEGES;");
  ...
  protected void createContainerGroup(Connection connection, String group) throws SQLException {
    executeQuery(
        connection,
        "CALL _SYS_DI.CREATE_CONTAINER_GROUP('" + group + "', _SYS_DI.T_NO_PARAMETERS, ?, ?, ?);");
  }

  // Grant Privileges to Container Group API
  protected void grantPrivilegesToContainerGroupAPI(Connection connection, String user)
      throws SQLException {
    executeUpdate(
        connection, "CREATE LOCAL TEMPORARY TABLE #PRIVILEGES LIKE _SYS_DI.TT_API_PRIVILEGES;");
    executeUpdate(
        connection,
        "INSERT INTO #PRIVILEGES (PRINCIPAL_NAME, PRIVILEGE_NAME, OBJECT_NAME) SELECT '"
            + user
  ...
  protected static Set<String> enumerateFolders(
      List<XSKDataStructureHDBCalculationViewModel> hdbCalculationViews) {
