...
public class XSKSecuritySynchronizer extends AbstractSynchronizer {
  ...
  private static final Map<String, List<XSKPrivilegeDefinition>> PRIVILEGES_PREDELIVERED =
      Collections.synchronizedMap(new HashMap<>());

  private static final Map<String, XSKAccessDefinition> ACCESS_PREDELIVERED =
      Collections.synchronizedMap(new HashMap<>());

  private static final Set<String> PRIVILEGES_SYNCHRONIZED =
      Collections.synchronizedSet(new HashSet<>());

  private static final Set<String> ACCESS_SYNCHRONIZED =
      Collections.synchronizedSet(new HashSet<>());

  @Inject private XSKAccessCoreService xskAccessCoreService;

  @Inject private XSKPrivilegeCoreService xskPrivilegeCoreService;
      ...
      if (beforeSynchronizing()) {
        ...
        try {
          ...
          successfulSynchronization(
              SYNCHRONIZER_NAME,
              format(
                  "Immutable: [Privileges: {0}, Access: {1}], Mutable: [Privileges: {2}, Access: {3}]",
                  immutablePrivilegesCount,
                  immutableAccessCount,
                  mutablePrivilegesCount,
                  mutableAccessCount));
        } catch (Exception e) {
          ...
          try {
            ...
            logger.error(
                "Synchronizing process for Privileges and Access artifacts failed in registering the state log.",
                e);
          }
        }
  ...
  public static final void forceSynchronization() {
    SecuritySynchronizer synchronizer =
        StaticInjector.getInjector().getInstance(SecuritySynchronizer.class);
  ...
  public void registerPredeliveredPrivileges(String privilegesPath) throws IOException {
    ...
    try {
      ...
      List<XSKPrivilegeDefinition> xskPrivilegeDefinitions =
          XSKPrivilegeArtifact.parse(json).divide();
  ...
  public void registerPredeliveredAccess(String xskAccessPath) throws IOException {
    ...
    try {
      String json = IOUtils.toString(in, StandardCharsets.UTF_8);
      XSKAccessDefinition xskAccessDefinition =
          XSKAccessArtifact.parse(json).toXSKAccessDefinition();
   ...
   * @throws SynchronizationException the synchronization exception
   */
  private void synchronizePrivilege(XSKPrivilegeDefinition xskPrivilegeDefinition)
      throws SynchronizationException {
    try {
      if (!xskPrivilegeCoreService.xskPrivilegeExists(xskPrivilegeDefinition.getName())) {
        xskPrivilegeCoreService.createXSKPrivilege(
            xskPrivilegeDefinition.getName(), xskPrivilegeDefinition.getDescription());
        logger.info("Synchronized a new XSK Privilege [{}]", xskPrivilegeDefinition.getName());
      } else {
        XSKPrivilegeDefinition existingXscPrivilegeDefinition =
            xskPrivilegeCoreService.getXSKPrivilegeByName(xskPrivilegeDefinition.getName());
        if (!existingXscPrivilegeDefinition
            .getDescription()
            .equals(xskPrivilegeDefinition.getDescription())) {
          xskPrivilegeCoreService.updateXSKPrivileges(
              xskPrivilegeDefinition.getName(), xskPrivilegeDefinition.getDescription());
          logger.info(
              "Synchronized a modified XSK Privilege [{}]", xskPrivilegeDefinition.getName());
        }
      }
   ...
   * @throws SynchronizationException the synchronization exception
   */
  private void synchronizeAccess(XSKAccessDefinition xskAccessDefinition)
      throws SynchronizationException {
    try {
      XSKAccessDefinition existingXSKAccessDefinition =
          xskAccessCoreService.getXSKAccessDefinition(xskAccessDefinition.getPath());
      if (existingXSKAccessDefinition == null) {
        xskAccessCoreService.createXSKAccessDefinition(
            xskAccessDefinition.getPath(),
            xskAccessDefinition.getAuthenticationMethod(),
            xskAccessDefinition.getHash(),
            xskAccessDefinition.isExposed(),
            xskAccessDefinition.getAuthorizationRolesAsList());
        logger.info(
            "Synchronized a new XSK Access definition [[{}]-[{}]] from location: {}",
            xskAccessDefinition.getAuthenticationMethod(),
            String.join(
                ",",
                (xskAccessDefinition.getAuthorizationRolesAsList() != null
                    ? xskAccessDefinition.getAuthorizationRolesAsList()
                    : new ArrayList())),
            xskAccessDefinition.getPath());
      } else if (!existingXSKAccessDefinition.getHash().equals(xskAccessDefinition.getHash())) {
        xskAccessCoreService.updateXSKAccessDefinition(
            xskAccessDefinition.getPath(),
            xskAccessDefinition.getAuthenticationMethod(),
            xskAccessDefinition.getHash(),
            xskAccessDefinition.isExposed(),
            xskAccessDefinition.getAuthorizationRolesAsList());
        logger.info(
            "Synchronized a modified XSK Access definition [[{}]-[{}]] from location: {}",
            xskAccessDefinition.getAuthenticationMethod(),
            String.join(
                ",",
                (xskAccessDefinition.getAuthorizationRolesAsList() != null
  ...
  protected void synchronizeResource(IResource resource) throws SynchronizationException {
    ...
    if (resourceName.equals(IXSKAccessCoreService.XSK_FILE_EXTENSION_ACCESS)) {
      XSKAccessDefinition xskAccessDefinition =
          xskAccessCoreService.parseXSAccessArtifact(resource.getContent());
  ...
  protected void cleanup() throws SynchronizationException {
    ...
    try {
      List<XSKPrivilegeDefinition> privilegeDefinitions =
          xskPrivilegeCoreService.getXSKPrivileges();
      for (XSKPrivilegeDefinition xskPrivilegeDefinition : privilegeDefinitions) {
      ...
      for (XSKAccessDefinition xskAccessDefinition : accessDefinitions) {
        if (!ACCESS_SYNCHRONIZED.contains(xskAccessDefinition.getPath())) {
          ...
          logger.warn(
              "Cleaned up XSK Access definition [[{}]-[{}]-[{}]] from location: {}",
              xskAccessDefinition.getAuthenticationMethod(),
              String.join(",", xskAccessDefinition.getAuthorizationRolesAsList()),
              xskAccessDefinition.getPath());
        }
      }
