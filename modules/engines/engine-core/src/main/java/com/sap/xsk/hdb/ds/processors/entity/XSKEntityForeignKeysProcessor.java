...
public class XSKEntityForeignKeysProcessor
    extends AbstractXSKProcessor<XSKDataStructureEntityModel> {
    ...
    for (XSKDataStructureHDBTableConstraintForeignKeyModel foreignKeyModel : foreignKeys) {
      ...
      if (existing) {
        AlterTableBuilder alterTableBuilder =
            SqlFactory.getNative(connection).alter().table(sourceTable);
        alterTableBuilder
            .add()
            .foreignKey(
                name,
                foreignKeyModel.getColumns(),
                tableName,
                foreignKeyModel.getReferencedColumns());
