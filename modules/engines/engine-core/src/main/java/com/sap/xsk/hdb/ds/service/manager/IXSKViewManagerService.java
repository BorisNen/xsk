...
@Singleton
public class IXSKViewManagerService
    extends AbstractDataStructureManagerService<XSKDataStructureHDBViewModel> {
  private static final Logger logger = LoggerFactory.getLogger(IXSKViewManagerService.class);

  private final Map<String, XSKDataStructureHDBViewModel> dataStructureViewsModels =
      new LinkedHashMap<>();
  ...
  @Override
  public void synchronizeRuntimeMetadata(XSKDataStructureHDBViewModel viewModel)
      throws XSKDataStructuresException {
    if (!getDataStructuresCoreService()
        .existsDataStructure(viewModel.getLocation(), viewModel.getType())) {
      ...
      logger.info(
          "Synchronized a new View file [{}] from location: {}",
          viewModel.getName(),
          viewModel.getLocation());
    } else {
      XSKDataStructureHDBViewModel existing =
          getDataStructuresCoreService()
              .getDataStructure(viewModel.getLocation(), viewModel.getType());
      if (!viewModel.equals(existing)) {
        ...
        dataStructureViewsModels.put(viewModel.getName(), viewModel);
        logger.info(
            "Synchronized a modified View file [{}] from location: {}",
            viewModel.getName(),
            viewModel.getLocation());
      }
    }
