...
public class XSKCSVRecordDao {
  ...
  public void save(CSVRecord csvRecord, String tableName, Boolean distinguishEmptyFromNull)
      throws SQLException {
    try (Connection connection = dataSource.getConnection()) {
      List<TableColumn> availableTableColumns =
          TableMetadataHelper.getColumns(connection, tableName);
      ...
      try (PreparedStatement statement = connection.prepareStatement(insertBuilder.generate())) {
        executeInsertPreparedStatement(
            distinguishEmptyFromNull, csvRecord, availableTableColumns, statement);
        logger.info(
            format(
                "Table row with id: %s was CREATED successfully in %s.",
                csvRecord.get(0), tableName));
      }
    }
  }

  public void update(
      CSVRecord csvRecord, String tableName, String pkValue, Boolean distinguishEmptyFromNull)
      throws SQLException {
    try (Connection connection = dataSource.getConnection()) {
      List<TableColumn> availableTableColumns =
          TableMetadataHelper.getColumns(connection, tableName);
      ...
      try (PreparedStatement statement = connection.prepareStatement(updateBuilder.generate())) {
        ...
        logger.info(
            format(
                "Table row with id: %s was UPDATED successfully in %s.",
                csvRecord.get(0), tableName));
      }
    }
  }
  ...
  public void deleteAll(List<String> ids, String tableName) throws SQLException {
    ...
    try (Connection connection = dataSource.getConnection()) {
      String pkColumnName =
          dbMetadataUtil.getTableMetadata(tableName).getColumns().get(0).getName();
      DeleteBuilder deleteBuilder = new DeleteBuilder(SqlFactory.deriveDialect(connection));
      deleteBuilder
          .from(tableName)
          .where(String.format("%s IN (%s)", pkColumnName, String.join(",", ids)));
      try (PreparedStatement statement = connection.prepareStatement(deleteBuilder.build())) {
        statement.executeUpdate();
        logger.info(
            String.format(
                "Entities with Row Ids: %s from table: %s", String.join(", ", ids), tableName));
      }
    }
  }
  ...
  private void executeInsertPreparedStatement(
  ...
  private void executeUpdatePreparedStatement(
      Boolean distinguishEmptyFromNull,
      CSVRecord csvRecord,
      List<TableColumn> availableTableColumns,
      PreparedStatement statement)
