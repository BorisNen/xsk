...
public class XSKEntityAlterProcessor {
  ...
  private static final String INCOMPATIBLE_CHANGE_OF_ENTITY =
      "Incompatible change of entity [%s] by adding a column [%s] which is [%s]"; //$NON-NLS-1$
    ...
    for (XSKDataStructureHDBTableColumnModel columnModel : entityModel.getColumns()) {
      ...
      if (!columnDefinitions.containsKey(name.toUpperCase())) {
        ...
        AlterTableBuilder alterTableBuilder =
            SqlFactory.getNative(connection).alter().table(tableName);

        alterTableBuilder.add().column(name, type, isPrimaryKey, isNullable, isUnique, args);

        if (!isNullable) {
          throw new SQLException(
              String.format(INCOMPATIBLE_CHANGE_OF_ENTITY, entityName, name, "NOT NULL"));
        }
        if (isPrimaryKey) {
          throw new SQLException(
              String.format(INCOMPATIBLE_CHANGE_OF_ENTITY, entityName, name, "PRIMARY KEY"));
        }
