...
public class XSKUtils {
  ...
  public static <T> T byteArrayToObject(byte[] byteArray)
   ...
   * hdbstructure, hdbprocedure) The catalog name includes the package path, the separating dots,
   * and the object base name, as NAMESPACE::OBJECT_BASE_NAME For example: Given location
   * "/projectname/com/sap/hana/example/ItemsByOrder.hdbview", the method will return
   ...
   * @see <a
   *     href="https://help.sap.com/viewer/52715f71adba4aaeb480d946c742d1f6/2.0.03/en-US/016a60fe929a4e9e89bbb3b6f7aad409.html">SAP
  ...
  public static String getRepositoryBaseObjectName(String location) {
    ...
    return new StringBuilder()
        .append(getRepositoryNamespace(location))
        .append("::")
        .append(objBaseName)
        .toString();
  }
  ...
  /**
   * Extraxt corret RepositoryBaseObject definition from content, by removing tabs or spaces between
   * the sybtaxx word and the Object name. For example "VIEW "hdb_view::ItemsByOrderHANAv2" should
   * be return as "VIEW "hdb_view::ItemsByOrderHANAv2"
   *
   ...
   */
  public static String extractRepositoryBaseObjectNameFromContent(
      String repositoryObjectSyntax, String content) {
    Pattern pattern =
        Pattern.compile(
            "(" + repositoryObjectSyntax + ")(\\t\\n)*(\\s)*(\".+\")", Pattern.CASE_INSENSITIVE);
