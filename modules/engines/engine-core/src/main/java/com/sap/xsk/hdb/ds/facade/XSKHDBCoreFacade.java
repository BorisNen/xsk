...
public class XSKHDBCoreFacade implements IXSKHDBCoreFacade {
  ...
  @javax.inject.Inject private DataSource dataSource;

  @Inject private Map<String, IXSKDataStructureManager> managerServices;
  ...
  public void handleResourceSynchronization(IResource resource)
    ...
    try {
      dataStructureModel =
          xskCoreParserService.parseDataStructure(resourceExtension, registryPath, contentAsString);
    ...
    managerServices
        .get(dataStructureModel.getType())
        .synchronizeRuntimeMetadata(dataStructureModel);
  }

  @Override
  public void handleResourceSynchronization(
      String fileExtension, XSKDataStructureModel dataStructureModel)
      throws SynchronizationException, XSKDataStructuresException {
    managerServices
        .get(dataStructureModel.getType())
        .synchronizeRuntimeMetadata(dataStructureModel);
  ...
  public void updateEntities() {
    Map<String, XSKDataStructureModel> dataStructureEntitiesModel =
        managerServices.get(IXSKDataStructureModel.TYPE_HDB_ENTITIES).getDataStructureModels();
    Map<String, XSKDataStructureModel> dataStructureTablesModel =
        managerServices.get(IXSKDataStructureModel.TYPE_HDB_TABLE).getDataStructureModels();
    Map<String, XSKDataStructureModel> dataStructureViewsModel =
        managerServices.get(IXSKDataStructureModel.TYPE_HDB_VIEW).getDataStructureModels();
    Map<String, XSKDataStructureModel> dataStructureProceduresModel =
        managerServices.get(IXSKDataStructureModel.TYPE_HDB_PROCEDURE).getDataStructureModels();
    Map<String, XSKDataStructureModel> dataStructureTableFunctionsModel =
        managerServices
            .get(IXSKDataStructureModel.TYPE_HDB_TABLE_FUNCTION)
            .getDataStructureModels();
    Map<String, XSKDataStructureModel> dataStructureSchemasModel =
        managerServices.get(IXSKDataStructureModel.TYPE_HDB_SCHEMA).getDataStructureModels();
    Map<String, XSKDataStructureModel> dataStructureSynonymModel =
        managerServices.get(IXSKDataStructureModel.TYPE_HDB_SYNONYM).getDataStructureModels();
    Map<String, XSKDataStructureModel> dataStructureSequencesModel =
        managerServices.get(IXSKDataStructureModel.TYPE_HDB_SEQUENCE).getDataStructureModels();
    ...
    try {
      ...
      try {
        ...
        boolean hdiOnly =
            Boolean.parseBoolean(Configuration.get(IXSKEnvironmentVariables.XSK_HDI_ONLY, "false"));
        if (!hdiOnly) {
          // drop HDB Sequences
          ...
          for (int i = sorted.size() - 1; i >= 0; i--) {
            ...
            XSKDataStructureHDBProcedureModel model =
                (XSKDataStructureHDBProcedureModel) dataStructureProceduresModel.get(dsName);
            if (model != null) {
              hdbProceduresToUpdate.add(model);
            }
          }

          IXSKDataStructureManager<XSKDataStructureModel> xskProceduresManagerService =
              managerServices.get(IXSKDataStructureModel.TYPE_HDB_PROCEDURE);
          for (XSKDataStructureHDBProcedureModel procedureToUpdate : hdbProceduresToUpdate) {
          ...
          for (int i = sorted.size() - 1; i >= 0; i--) {
            ...
            XSKDataStructureHDBTableFunctionModel model =
                (XSKDataStructureHDBTableFunctionModel)
                    dataStructureTableFunctionsModel.get(dsName);
            if (model != null) {
              hdbTableFunctionsToUpdate.add(model);
            }
          }

          IXSKDataStructureManager<XSKDataStructureModel> xskTableFunctionManagerService =
              managerServices.get(IXSKDataStructureModel.TYPE_HDB_TABLE_FUNCTION);
          ...
          IXSKDataStructureManager<XSKDataStructureModel> xskViewManagerService =
              managerServices.get(IXSKDataStructureModel.TYPE_HDB_VIEW);

          for (int i = sorted.size() - 1; i >= 0; i--) {
            String dsName = sorted.get(i);
            XSKDataStructureHDBViewModel model =
                (XSKDataStructureHDBViewModel) dataStructureViewsModel.get(dsName);
            try {
              if (model != null) {
          ...
          IXSKDataStructureManager<XSKDataStructureModel> xskTableManagerService =
              managerServices.get(IXSKDataStructureModel.TYPE_HDB_TABLE);

          // drop tables in a reverse order
          for (int i = sorted.size() - 1; i >= 0; i--) {
            String dsName = sorted.get(i);
            XSKDataStructureHDBTableModel model =
                (XSKDataStructureHDBTableModel) dataStructureTablesModel.get(dsName);
            try {
              ...
              if (model != null && SqlFactory.getNative(connection).exists(connection, modelName)) {
                if (SqlFactory.getNative(connection).count(connection, modelName) == 0) {
                  ...
                  logger.warn(
                      format(
                          "Table [{0}] cannot be deleted during the update process, because it is not empty",
                          dsName));
                }
              }
            } catch (Exception e) {
          ...
          IXSKDataStructureManager<XSKDataStructureModel> xskEntityManagerService =
              managerServices.get(IXSKDataStructureModel.TYPE_HDB_ENTITIES);

          // drop entities in a reverse order
          for (int i = sorted.size() - 1; i >= 0; i--) {
            String dsName = sorted.get(i);
            XSKDataStructureEntitiesModel entitiesModel =
                (XSKDataStructureEntitiesModel) dataStructureEntitiesModel.get(dsName);
            try {
          ...
          IXSKDataStructureManager<XSKDataStructureModel> xskSynonymManagerService =
              managerServices.get(IXSKDataStructureModel.TYPE_HDB_SYNONYM);
          for (int i = sorted.size() - 1; i >= 0; i--) {
            String dsName = sorted.get(i);
            XSKDataStructureHDBSynonymModel model =
                (XSKDataStructureHDBSynonymModel) dataStructureSynonymModel.get(dsName);
            try {
              if (model != null) {
          ...
          for (int i = sorted.size() - 1; i >= 0; i--) {
            ...
            XSKDataStructureHDBSchemaModel model =
                (XSKDataStructureHDBSchemaModel) dataStructureSchemasModel.get(dsName);
            if (model != null) {
              hdbSchemasToUpdate.add(model);
            }
          }

          IXSKDataStructureManager<XSKDataStructureModel> xskSchemaManagerService =
              managerServices.get(IXSKDataStructureModel.TYPE_HDB_SCHEMA);
          // executeHDBSchemasDrop(connection, hdbSchemasToUpdate);
          for (XSKDataStructureHDBSchemaModel schemaModel : hdbSchemasToUpdate) {
            xskSchemaManagerService.dropDataStructure(connection, schemaModel);
          }

          // process sequences in the proper order
          IXSKDataStructureManager<XSKDataStructureModel> xskSequenceManagerService =
              managerServices.get(IXSKDataStructureModel.TYPE_HDB_SEQUENCE);
          for (String dsName : sorted) {
            XSKDataStructureHDBSequenceModel model =
                (XSKDataStructureHDBSequenceModel) dataStructureSequencesModel.get(dsName);
            try {
              if (model != null) {
                if (!SqlFactory.getNative(connection)
                    .exists(connection, model.getName(), DatabaseArtifactTypes.SEQUENCE)) {
            ...
            try {
              if (model != null) {
                if (!SqlFactory.getNative(connection).exists(connection, model.getName())) {
                  ...
                  logger.warn(
                      format("Table [{0}] already exists during the update process", dsName));
          ...
          for (String dsName : sorted) {
            XSKDataStructureEntitiesModel entitesModel =
                (XSKDataStructureEntitiesModel) dataStructureEntitiesModel.get(dsName);
            try {
            ...
            try {
              if (model != null) {
                if (!SqlFactory.getNative(connection).exists(connection, model.getName())) {
                  ...
                  logger.warn(
                      format("View [{0}] already exists during the update process", dsName));
                }
              }
            } catch (Exception e) {
              logger.error(e.getMessage(), e);
            ...
            try {
              if (model != null) {
                if (!SqlFactory.getNative(connection).exists(connection, model.getName())) {
                  ...
                  logger.warn(
                      format("Synonym [{0}] already exists during the update process", dsName));
                }
              }
            } catch (Exception e) {
              logger.error(e.getMessage(), e);
          ...
          for (XSKDataStructureHDBProcedureModel procedureModel : hdbProceduresToUpdate) {
  ...
  private String getRegistryPath(IResource resource) {
    ...
    return resourcePath.startsWith("/registry/public")
        ? resourcePath.substring("/registry/public".length())
        : resourcePath;
  }
