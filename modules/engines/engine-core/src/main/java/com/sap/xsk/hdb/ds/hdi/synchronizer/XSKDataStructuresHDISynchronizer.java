...
public class XSKDataStructuresHDISynchronizer extends AbstractSynchronizer {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(XSKDataStructuresHDISynchronizer.class);

  private static final Map<String, XSKDataStructureHDIModel> HDI_PREDELIVERED =
      Collections.synchronizedMap(new HashMap<>());

  private static final List<String> HDI_SYNCHRONIZED =
      Collections.synchronizedList(new ArrayList<String>());

  private static final Map<String, XSKDataStructureHDIModel> DATA_STRUCTURE_HDI_MODELS =
      new LinkedHashMap<String, XSKDataStructureHDIModel>();
  ...
  @Inject private XSKHDIContainerCreateProcessor xskhdiContainerCreateProcessor;
        ...
        try {
          if (isSynchronizerSuccessful(
            ...
            successfulSynchronization(
                SYNCHRONIZER_NAME,
                format("Immutable: {0}, Mutable: {1}", immutableHDICount, mutableHDICount));
          } else {
          ...
          try {
            ...
            logger.error(
                "Synchronizing process for HDB Data Structures HDI files failed in registering the state log.",
                e);
          }
        }
  ...
  public static final void forceSynchronization() {
    XSKDataStructuresHDISynchronizer synchronizer =
        StaticInjector.getInjector().getInstance(XSKDataStructuresHDISynchronizer.class);
  ...
  public void registerPredeliveredHDI(String contentPath) throws Exception {
    ...
    XSKDataStructureHDIModel model =
        (XSKDataStructureHDIModel)
            xskCoreParserService.parseDataStructure(
                IXSKDataStructureModel.TYPE_HDI, contentPath, data);
  ...
  private void synchronizePredelivered() throws SynchronizationException {
    ...
    for (XSKDataStructureHDIModel hdi : HDI_PREDELIVERED.values()) {
      try {
        ...
        logger.error(
            format("Update hdi [{0}] skipped due to an error: {1}", hdi, e.getMessage()), e);
      }
    }
  ...
  private void synchronizeHDI(XSKDataStructureHDIModel hdi) throws SynchronizationException {
    try {
      if (!xskDataStructuresCoreService.existsDataStructure(hdi.getLocation(), hdi.getType())) {
        xskDataStructuresCoreService.createDataStructure(
            hdi.getLocation(), hdi.getName(), hdi.getHash(), hdi.getType());
        DATA_STRUCTURE_HDI_MODELS.put(hdi.getName(), hdi);
        logger.info(
            "Synchronized a new HDI file [{}] from location: {}", hdi.getName(), hdi.getLocation());
      } else {
        XSKDataStructureHDIModel existing =
            xskDataStructuresCoreService.getDataStructure(hdi.getLocation(), hdi.getType());
        if (!hdi.equals(existing)) {
          xskDataStructuresCoreService.updateDataStructure(
              hdi.getLocation(), hdi.getName(), hdi.getHash(), hdi.getType());
          DATA_STRUCTURE_HDI_MODELS.put(hdi.getName(), hdi);
          logger.info(
              "Synchronized a modified HDI file [{}] from location: {}",
              hdi.getName(),
              hdi.getLocation());
        }
      }
  ...
  protected void synchronizeResource(IResource resource) throws SynchronizationException {
    ...
    boolean hdiSupported =
        Boolean.parseBoolean(Configuration.get(IXSKEnvironmentVariables.XSK_HDI_SUPPORTED, "true"));
    if (hdiSupported) {
      if (resourceName.endsWith(IXSKDataStructureModel.FILE_EXTENSION_HDI)) {
        ...
        try {
          hdi =
              (XSKDataStructureHDIModel)
                  xskCoreParserService.parseDataStructure(
                      IXSKDataStructureModel.TYPE_HDI, registryPath, contentAsString);
        } catch (XSKDataStructuresException e) {
  ...
  protected void cleanup() throws SynchronizationException {
    ...
    try {
      ...
      try {
        ...
        List<XSKDataStructureHDIModel> hdiModels =
            xskDataStructuresCoreService.getDataStructuresByType(IXSKDataStructureModel.TYPE_HDI);
        for (XSKDataStructureHDIModel hdiModel : hdiModels) {
          if (!HDI_SYNCHRONIZED.contains(hdiModel.getLocation())) {
            ...
            logger.warn(
                "Cleaned up HDI Container file [{}] from location: {}",
                hdiModel.getName(),
                hdiModel.getLocation());
          }
        }
      ...
      try {
        ...
        boolean hdiSupported =
            Boolean.parseBoolean(
                Configuration.get(IXSKEnvironmentVariables.XSK_HDI_SUPPORTED, "true"));
        if (hdiSupported) {
          // HDI Containers
  ...
  private void executeHDI(Connection connection, List<XSKDataStructureHDIModel> hdiModels)
      throws SQLException {
    hdiModels.forEach(
        hdiModel -> this.xskhdiContainerCreateProcessor.execute(connection, hdiModel));
  }
}
