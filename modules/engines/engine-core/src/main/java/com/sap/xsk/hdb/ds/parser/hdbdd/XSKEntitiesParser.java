...
public class XSKEntitiesParser implements XSKDataStructureParser {
  ...
  @Override
  public XSKDataStructureEntitiesModel parse(String location, String content)
    ...
    for (Iterator iterator = hdbDD.getElements().iterator(); iterator.hasNext(); ) {
      com.sap.xsk.models.hdbdd.hdbDD.Type type =
          (com.sap.xsk.models.hdbdd.hdbDD.Type) iterator.next();
      if (type instanceof NamespaceImpl) {
        ...
        List<XSKDataStructureHDBTableConstraintForeignKeyModel> foreignKeys =
            new ArrayList<XSKDataStructureHDBTableConstraintForeignKeyModel>();
        for (Iterator iteratorEntries = eContext.getEntities().iterator();
            iteratorEntries.hasNext(); ) {
          ...
          if (entry instanceof EntityImpl) {
            ...
            for (Iterator iteratorColumns = eEntity.getFields().iterator();
                iteratorColumns.hasNext(); ) {
              ...
              if (eField instanceof FieldPrimitiveImpl) {
                ...
                XSKDataStructureHDBTableColumnModel column =
                    new XSKDataStructureHDBTableColumnModel();
                // column.setName(CaseFormat.UPPER_CAMEL.to(CaseFormat.UPPER_UNDERSCORE,
                // eFieldPrimitive.getName()));
                ...
                XSKDataStructureTypeDefinitionModel typeDefinition =
                    context.getTypes().get(eFieldType.getName());
                if (typeDefinition != null) {
                  ...
                  throw new IllegalArgumentException(
                      "Undefined field type: " + eFieldType.getName());
                }
              } else if (eField instanceof FieldReferenceImpl) {
                FieldReferenceImpl eFieldReference = (FieldReferenceImpl) eField;
                XSKDataStructureHDBTableConstraintForeignKeyModel referenceDefinition =
                    new XSKDataStructureHDBTableConstraintForeignKeyModel();
                referenceDefinition.setName(eFieldReference.getFieldReferenceEntity());
                referenceDefinition.setReferencedTable(entity.getName());
                String fieldReferenceEntityLocalId =
                    eFieldReference.getFieldReferenceEntityLocalId();
                fieldReferenceEntityLocalId =
                    fieldReferenceEntityLocalId.substring(
                        fieldReferenceEntityLocalId.indexOf(".") + 1);
                referenceDefinition.setColumns(new String[] {fieldReferenceEntityLocalId});
                referenceDefinition.setReferencedColumns(
                    new String[] {eFieldReference.getFieldReferenceEntityForeignId()});
                foreignKeys.add(referenceDefinition);
              } else {
                throw new IllegalArgumentException(
                    "Unknown field type: " + eField.getClass().getCanonicalName());
              }
            }
            ...
            XSKDataStructureTypeDefinitionModel typeDefinition =
                new XSKDataStructureTypeDefinitionModel(
                    eTypeDefinition.getName(),
                    eTypeDefinition.getFieldType(),
                    eTypeDefinition.getFieldLength());
            context.getTypes().put(typeDefinition.getName(), typeDefinition);
          } else {
            throw new IllegalArgumentException(
                "Unknown field entry: " + entry.getClass().getCanonicalName());
          }
        }
