...
public class IXSKTableManagerService
    extends AbstractDataStructureManagerService<XSKDataStructureHDBTableModel> {
  ...
  public void synchronizeRuntimeMetadata(XSKDataStructureHDBTableModel tableModel)
    ...
    if (!getDataStructuresCoreService()
        .existsDataStructure(tableModel.getLocation(), tableModel.getType())) {
      ...
      logger.info(
          "Synchronized a new Table file [{}] from location: {}",
          tableModel.getName(),
          tableModel.getLocation());
    } else {
      XSKDataStructureHDBTableModel existing =
          getDataStructuresCoreService()
      ...
      if (!tableModel.equals(existing)) {
        ...
        dataStructureTableModels.put(tableModel.getName(), tableModel);
        logger.info(
            "Synchronized a modified Table file [{}] from location: {}",
            tableModel.getName(),
            tableModel.getLocation());
      }
    }
