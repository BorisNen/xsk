...
public class XSKODataSynchronizer extends AbstractSynchronizer {
  ...
  private static final Map<String, XSKODataModel> ODATA_PREDELIVERED =
      Collections.synchronizedMap(new HashMap<String, XSKODataModel>());

  private static final List<String> ODATA_SYNCHRONIZED =
      Collections.synchronizedList(new ArrayList<String>());

  private static final Map<String, XSKODataModel> ODATA_MODELS =
      new LinkedHashMap<String, XSKODataModel>();

  @Inject private XSKODataCoreService xskODataCoreService;

  @Inject private ODataCoreService odataCoreService;

  @Inject private DataSource dataSource;

  @Inject private XSKOData2ODataMTransformer xskOData2ODataMTransformer;

  @Inject private XSKOData2ODataXTransformer xskOData2ODataXTransformer;
      ...
      if (beforeSynchronizing()) {
        ...
        try {
          ...
          try {
            ...
            logger.error(
                "Synchronizing process for XSOData files failed in registering the state log.", e);
          }
        }
  ...
  public static final void forceSynchronization() {
    XSKODataSynchronizer synchronizer =
        StaticInjector.getInjector().getInstance(XSKODataSynchronizer.class);
  ...
  private void synchronizePredelivered() throws SynchronizationException {
    ...
    for (XSKODataModel odata : ODATA_PREDELIVERED.values()) {
      try {
        ...
        logger.error(
            format("Update xsodata [{0}] skipped due to an error: {1}", odata, e.getMessage()), e);
      }
    }
  ...
  private void synchronizeOData(XSKODataModel odataModel) throws SynchronizationException {
    try {
      if (!xskODataCoreService.existsOData(odataModel.getLocation())) {
        xskODataCoreService.createOData(
            odataModel.getLocation(), odataModel.getName(), odataModel.getHash());
        ODATA_MODELS.put(odataModel.getName(), odataModel);
        logger.info(
            "Synchronized a new XSOData file [{}] from location: {}",
            odataModel.getName(),
            odataModel.getLocation());
      } else {
        XSKODataModel existing = xskODataCoreService.getOData(odataModel.getLocation());
        if (!odataModel.equals(existing)) {
          xskODataCoreService.updateOData(
              odataModel.getLocation(), odataModel.getName(), odataModel.getHash());
          ODATA_MODELS.put(odataModel.getName(), odataModel);
          logger.info(
              "Synchronized a modified XSOData file [{}] from location: {}",
              odataModel.getName(),
              odataModel.getLocation());
        }
      }
      ...
      try {
        ...
        for (XSKODataModel odataModel : odataModels) {
          if (!ODATA_SYNCHRONIZED.contains(odataModel.getLocation())) {
            ...
            logger.warn(
                "Cleaned up XSOData Data file [{}] from location: {}",
                odataModel.getName(),
                odataModel.getLocation());
          }
        }
