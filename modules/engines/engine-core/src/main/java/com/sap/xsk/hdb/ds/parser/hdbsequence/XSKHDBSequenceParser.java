...
public class XSKHDBSequenceParser implements XSKDataStructureParser {
  ...
  public XSKDataStructureModel parse(String location, String content)
    ...
    String expectedHana2Syntax =
        XSKConstants.XSK_HDBSEQUENCE_SYNTAX
            + "\""
            + XSKUtils.getRepositoryBaseObjectName(location)
            + "\"";
    String receivedSyntax =
        XSKUtils.extractRepositoryBaseObjectNameFromContent(
            XSKConstants.XSK_HDBSEQUENCE_SYNTAX, content);
    logger.debug(
        "Determine if the hdbsequence is Hana v1 or v2 by Comparing '"
            + receivedSyntax
            + "' with '"
            + expectedHana2Syntax
            + "'");
  ...
  private XSKDataStructureModel parseHANAv1Content(String location, String content)
      throws XSKDataStructuresException, IOException {
    ...
    Gson gson =
        new GsonBuilder()
            .registerTypeAdapter(
                XSKHDBSEQUENCEModel.class, new CustomDeserializers.XSKHDBSEQUENCEModelAdapter())
            .create();
    ...
    XSKDataStructureHDBSequenceModel hdbSequenceModel =
        modelMapper.map(antlr4Model, XSKDataStructureHDBSequenceModel.class);
    setXSKDataStructureHDBSequenceModelTrackingDetails(
        location, content, XSKHanaVersion.VERSION_1, hdbSequenceModel);
  ...
  private XSKDataStructureModel parseHANAv2Content(String location, String content) {
    ...
    setXSKDataStructureHDBSequenceModelTrackingDetails(
        location, content, XSKHanaVersion.VERSION_2, hdbSequenceModel);
    hdbSequenceModel.setRawContent(content);
    return hdbSequenceModel;
  }

  private void setXSKDataStructureHDBSequenceModelTrackingDetails(
      String location,
      String content,
      XSKHanaVersion hanaVersion,
      XSKDataStructureHDBSequenceModel hdbSequenceModel) {
