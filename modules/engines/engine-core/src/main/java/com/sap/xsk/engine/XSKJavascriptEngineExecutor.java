...
public class XSKJavascriptEngineExecutor extends GraalVMJavascriptEngineExecutor
  ...
  private XSKRepositoryModuleSourceProvider sourceProvider =
      new XSKRepositoryModuleSourceProvider(this, IRepositoryStructure.PATH_REGISTRY_PUBLIC);
  ...
  protected void beforeEval(Context context) throws IOException {
    ...
    context
        ...
        .putMember("$", context.eval(ENGINE_JAVA_SCRIPT, xskApi));
  ...
  private String getXskApi() throws IOException {
    if (XSK_API_CONTENT == null) {
      XSK_API_CONTENT =
          IOUtils.toString(
              XSKJavascriptEngineExecutor.class.getResourceAsStream(XSK_API_LOCATION),
              DEFAULT_CHARSET);
    }
  ...
  //	try {
  //	ModuleSource moduleSource = (isModule ? sourceProvider.loadSource(moduleOrCode, null, null) :
  // null);
  //	try {
  //		// load the HANA XS Classic Bridge
  //		String api =
  // IOUtils.toString(XSKJavascriptEngineExecutor.class.getResourceAsStream("/xsk/api.js"),
  // Charset.defaultCharset());
  //		context.evaluateString(topLevelScope, api, "$", -1, null);
  //
  //		if (moduleSource != null) {
  //			result = context.evaluateReader(topLevelScope, moduleSource.getReader(), moduleOrCode, -1,
  // null);
  ...
  //	private void callXSKJobFunction(Scriptable topLevelScope, Context context, Map<Object, Object>
  // executionContext) {
  ...
  //		Object[] functionParameters = (Object[])
  // executionContext.get(IXSKJobCoreService.XSK_JOB_PARAMETERS);
