...
public class XSKViewParser implements XSKDataStructureParser {
  ...
  public XSKDataStructureHDBViewModel parse(String location, String content)
    ...
    String expectedHana2Syntax =
        XSKConstants.XSK_HDBVIEW_SYNTAX
            + "\""
            + XSKUtils.getRepositoryBaseObjectName(location)
            + "\"";
    String receivedHana2Syntax =
        XSKUtils.extractRepositoryBaseObjectNameFromContent(
            XSKConstants.XSK_HDBVIEW_SYNTAX, content);
    logger.debug(
        "Determine if the hdbview is Hana v1 or v2 by Comparing '"
            + receivedHana2Syntax
            + "' with '"
            + expectedHana2Syntax
            + "'");
  ...
  private XSKDataStructureHDBViewModel parseHANAv1Content(String location, String content)
      throws XSKDataStructuresException, IOException {
    XSKDataStructureHDBViewModel hdbViewModel = new XSKDataStructureHDBViewModel();
    populateXSKDataStructureHDBViewModel(location, content, hdbViewModel);
    ...
    XSKHDBVIEWSyntaxErrorListener xskhdbviewSyntaxErrorListener =
        new XSKHDBVIEWSyntaxErrorListener();
    ...
    try {
      ...
      throw new XSKDataStructuresException(
          String.format(
              "Wrong format of HDB View: [%s] during parsing. [%s]", location, e.getMessage()));
    }
  ...
  private void populateXSKDataStructureHDBViewModel(
      String location, String content, XSKDataStructureHDBViewModel hdbViewModel) {
