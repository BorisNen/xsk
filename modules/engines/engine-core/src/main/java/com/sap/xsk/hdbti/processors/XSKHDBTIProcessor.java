...
public class XSKHDBTIProcessor implements IXSKHDBTIProcessor {
  ...
  public void process(
    ...
    IResource resource =
        repository.getResource(
            xskCsvToHdbtiRelationDao.convertToActualFileName(
                tableImportConfigurationDefinition.getFile()));
    String tableName =
        xskTableImportParser.convertToActualTableName(
            tableImportConfigurationDefinition.getTable());
    ...
    Map<String, XSKImportedCSVRecordModel> importedCSVRecordsWithPk =
        xskImportedCSVRecordDao.getImportedCSVRecordsByTableAndCSVLocation(
            tableName, tableImportConfigurationDefinition.getFile());
    ...
    for (CSVRecord csvRecord : csvParser) {
      ...
      if (pkForCSVRecord != null && !importedCSVRecordsWithPk.containsKey(pkForCSVRecord)) {
        recordsToInsert.add(csvRecord);
          ...
          && !importedCSVRecordsWithPk.get(pkForCSVRecord).getHash().equals(csvRecordHash)) {
        recordsToUpdate.add(csvRecord);
        XSKImportedCSVRecordModel importedCSVRecordModelToUpdate =
            importedCSVRecordsWithPk.get(pkForCSVRecord);
    ...
    for (CSVRecord csvRecord : recordsToUpdate) {
      try {
        xskcsvRecordDao.update(
            csvRecord,
            tableName,
            getPkForCSVRecord(csvRecord, tableMetadata),
    ...
    List<String> importedIdsToRemove =
        importedCSVRecordsWithPk.values().stream()
            .map(XSKImportedCSVRecordModel::getId)
            .map(Object::toString)
            .collect(Collectors.toList());
    List<String> idsToRemove =
        importedCSVRecordsWithPk.values().stream()
            .map(csv -> csv.getRowId())
            .collect(Collectors.toList());
    try {
      ...
      logger.error(
          String.format(
              "An error occurred while trying to delete removed CSVs with ids: %s from table %s",
              String.join(",", importedIdsToRemove)));
    }
  }

  private void insertCsvRecords(
      XSKTableImportConfigurationDefinition tableImportConfigurationDefinition,
      String tableName,
      PersistenceTableModel tableMetadata,
      List<CSVRecord> recordsToInsert) {
    for (CSVRecord csvRecord : recordsToInsert) {
      try {
        xskcsvRecordDao.save(
            csvRecord, tableName, tableImportConfigurationDefinition.getDistinguishEmptyFromNull());
        ...
        importedCSVRecordModel.setHdbtiLocation(
        ...
        logger.error(
            "Error occurred while inserting the csv values in the table pointed by HDBTI file", e);
        logger.error(
            format(
                "Error occurred while processing %s for table %s at record %d",
                tableImportConfigurationDefinition.getFile(),
                tableImportConfigurationDefinition.getTable(),
  ...
  private PersistenceTableModel getTableMetadata(
      XSKTableImportConfigurationDefinition tableImportConfigurationDefinition) {
    String tableName =
        xskTableImportParser.convertToActualTableName(
            tableImportConfigurationDefinition.getTable());
    try {
      ...
      logger.error(
          String.format(
              "Error occurred while trying to read table metadata for table with name: %s",
              tableName),
          sqlException);
    }
      ...
      String contentAsString =
          IOUtils.toString(
              new InputStreamReader(
                  new ByteArrayInputStream(resource.getContent()), StandardCharsets.UTF_8));
      CSVFormat csvFormat = createCSVFormat(tableImportConfigurationDefinition);
      return CSVParser.parse(contentAsString, csvFormat);
    } catch (IOException e) {
      logger.error(
          String.format(
              "Error occurred while trying to parse csv imported from hdbti file: %s",
              tableImportConfigurationDefinition.getHdbtiFileName()),
          e);
    }
  ...
  private CSVFormat createCSVFormat(
      XSKTableImportConfigurationDefinition tableImportConfigurationDefinition) {
    char delimiter =
        Objects.isNull(tableImportConfigurationDefinition.getDelimField())
            ? ','
