...
@WebFilter(
    ...
    filterName = "XSKSecurityFilter",
    description = "Check all the URIs for access permissions")
public class XSKSecurityFilter implements Filter {
  ...
  private static IXSKAccessCoreService xskAccessCoreService =
      StaticInjector.getInjector().getInstance(XSKAccessCoreService.class);
    ...
    try {
      ...
      if (!path.startsWith(PATH_WEB_RESOURCES)) {
        ...
        XSKAccessDefinition xskAccessDefinition =
            XSKAccessVerifier.getMatchingAccessDefinitions(xskAccessCoreService, path, method);
        ...
        if (xskAccessDefinition != null) {
          ...
          if (principal == null && !Configuration.isJwtModeEnabled()) {
            ...
            if (!isInRole) {
              forbidden(
                  path,
                  "The logged in user does not have any of the required roles for the requested URI",
                  httpServletResponse);
              return;
            }
          }
          ...
          if (!Configuration.isAnonymousModeEnabled()
              && principal == null
              && !Configuration.isJwtModeEnabled()) {
   ...
   */
  private void forbidden(String uri, String message, HttpServletResponse response)
      throws IOException {
