...
public class XSKSecureStoreCoreService implements IXSKSecureStoreCoreService {
  ...
  @Inject private PersistenceManager<XSKSecureStore> secureStorePersistenceManager;

  @Inject private PersistenceManager<XSKSecureStoreContent> secureStoreContentPersistenceManager;

  @Inject private XSKSecureStoreEncryptor xskSecureStoreEncryptor;

  @Override
  public XSKSecureStore createSecureStore(String location, String content)
      throws XSKSecureStoreException {
  ...
  @Override
  public void createSecureStoreValue(String storeId, String userId, String dataId, String value)
      throws XSKSecureStoreException {
    ...
    XSKSecureStoreContent existingXscSecureStoreContent =
        findSecureStoreContent(storeId, userId, dataId);
  ...
  @Override
  public void updateSecureStoreValue(XSKSecureStoreContent xskSecureStoreContent)
      throws XSKSecureStoreException {
    try {
      Connection connection = null;
      try {
  ...
  @Override
  public XSKSecureStoreContent findSecureStoreContent(String storeId, String userId, String dataId)
      throws XSKSecureStoreException {
    ...
    try {
      ...
      try {
        connection = dataSource.getConnection();
        List<XSKSecureStoreContent> foundContent =
            secureStoreContentPersistenceManager.query(
                connection,
                XSKSecureStoreContent.class,
                IXSKSecureStoreModel.SECURE_STORE_VALUE_FIND_STATEMENT,
                queryArguments);
  ...
  @Override
  public void deleteSecureStoreValue(String storeId, String userId, String dataId)
      throws XSKSecureStoreException {
    if (!existsSecureStoreContent(storeId, userId, dataId)) {
      throw new XSKSecureStoreException(
          "No such secure createSecureStoreValue value to be removed.");
    }
    ...
    try {
      ...
      try {
        connection = dataSource.getConnection();
        secureStoreContentPersistenceManager.execute(
            connection, IXSKSecureStoreModel.SECURE_STORE_VALUE_DELETE_STATEMENT, queryArguments);
      } finally {
        if (connection != null) {
          connection.close();
        }
  ...
  public void deleteSecureStoreValuesByStoreId(String userId) throws XSKSecureStoreException {
    ...
    try {
      ...
      try {
        ...
        secureStoreContentPersistenceManager.execute(
            connection,
            IXSKSecureStoreModel.SECURE_STORE_DELETE_BY_STORE_ID,
  ...
  @Override
  public boolean existsSecureStoreContent(String storeId, String userId, String dataId)
      throws XSKSecureStoreException {
