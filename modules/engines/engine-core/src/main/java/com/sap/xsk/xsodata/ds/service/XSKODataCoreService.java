...
public class XSKODataCoreService implements IXSKODataCoreService {
  ...
  @Inject private XSKODataModelFactory factory;

  @Inject private PersistenceManager<XSKODataModel> odataPersistenceManager;
   ...
   */
  @Override
  public XSKODataModel createOData(String location, String name, String hash)
      ...
      try {
        ...
        String sql =
            SqlFactory.getNative(connection)
                ...
                .where("OD_NAME = ?")
                .toString();
        List<XSKODataModel> tableModels =
            odataPersistenceManager.query(
                connection, XSKODataModel.class, sql, Arrays.asList(name));
        if (tableModels.isEmpty()) {
          return null;
        }
        if (tableModels.size() > 1) {
          throw new XSKODataException(
              format(
                  "There are more that one OData with the same name [{0}] at locations: [{1}] and [{2}].",
  ...
  public List<XSKODataModel> getODatas() throws XSKODataException {
    try {
      ...
      try {
        connection = dataSource.getConnection();
        String sql =
            SqlFactory.getNative(connection).select().column("*").from("XSK_ODATA").toString();
        List<XSKODataModel> tableModels =
            odataPersistenceManager.query(connection, XSKODataModel.class, sql);
        return tableModels;
      } finally {
        if (connection != null) {
          connection.close();
  ...
  public static XSKODataEntity getEntity(XSKODataModel model, String alias, String navigation) {
    ...
    throw new IllegalArgumentException(
        String.format(
            "There is no entity with name: %s, referenced by the navigation: %s",
            alias, navigation));
  }

  public static XSKODataAssociation getAssociation(
      XSKODataModel model, String name, String navigation) {
    ...
    throw new IllegalArgumentException(
        String.format(
            "There is no association with name: %s, referenced by the navigation: %s",
            name, navigation));
  }
}
