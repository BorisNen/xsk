...
public class IXSKHDBSequenceManagerService
    extends AbstractDataStructureManagerService<XSKDataStructureHDBSequenceModel> {
  ...
  @Override
  public void synchronizeRuntimeMetadata(XSKDataStructureHDBSequenceModel hdbSequenceModel)
      throws XSKDataStructuresException {
    if (!getDataStructuresCoreService()
        .existsDataStructure(hdbSequenceModel.getLocation(), hdbSequenceModel.getType())) {
      getDataStructuresCoreService()
          .createDataStructure(
              hdbSequenceModel.getLocation(),
              hdbSequenceModel.getName(),
              hdbSequenceModel.getHash(),
              hdbSequenceModel.getType());
      dataStructureSequenceModels.put(hdbSequenceModel.getName(), hdbSequenceModel);
      logger.info(
          "Synchronized a new Hdbsequence file [{}] from location: {}",
          hdbSequenceModel.getName(),
          hdbSequenceModel.getLocation());
    } else {
      XSKDataStructureHDBSequenceModel existing =
          getDataStructuresCoreService()
      ...
      if (!hdbSequenceModel.equals(existing)) {
        ...
        dataStructureSequenceModels.put(hdbSequenceModel.getName(), hdbSequenceModel);
        logger.info(
            "Synchronized a modified Hdbsequence file [{}] from location: {}",
            hdbSequenceModel.getName(),
            hdbSequenceModel.getLocation());
      }
    }
  ...
  public void createDataStructure(
      Connection connection, XSKDataStructureHDBSequenceModel hdbSequenceModel)
      throws SQLException {
