...
public class XSKJobSynchronizer extends AbstractSynchronizer {
  ...
  private static final Map<String, XSKJobDefinition> JOBS_PREDELIVERED =
      Collections.synchronizedMap(new HashMap<String, XSKJobDefinition>());

  private static final List<String> JOBS_SYNCHRONIZED =
      Collections.synchronizedList(new ArrayList<String>());

  @Inject private XSKJobCoreService schedulerCoreService;

  @Inject private XSKJobToXSKJobDefinitionTransformer xskJobToXSKJobDefinitionTransformer;
      ...
      if (beforeSynchronizing()) {
        ...
        try {
          ...
          try {
            ...
            logger.error(
                "Synchronizing process for Jobs files failed in registering the state log.", e);
          }
        }
  ...
  public static final void forceSynchronization() {
    XSKJobSynchronizer synchronizer =
        StaticInjector.getInjector().getInstance(XSKJobSynchronizer.class);
  ...
  public void registerPredeliveredJob(String jobPath) throws IOException, ParseException {
    ...
    try {
      ...
      XSKJobDefinition jobDefinition =
  ...
  private void synchronizeJob(XSKJobDefinition xskJob) throws SynchronizationException {
    try {
      if (!schedulerCoreService.existsJob(xskJob.getName())) {
        schedulerCoreService.createJob(
            xskJob.getName(),
            xskJob.getGroup(),
            xskJob.getDescription(),
            xskJob.getModule(),
            xskJob.getFunction(),
            xskJob.getCronExpression(),
            xskJob.getParametersAsMap());
        logger.info(
            "Synchronized a new Job [{}] from group: {}", xskJob.getName(), xskJob.getGroup());
      } else {
        XSKJobDefinition existing = schedulerCoreService.getJob(xskJob.getName());
        if (!xskJob.equals(existing)) {
          schedulerCoreService.updateJob(
              xskJob.getName(),
              xskJob.getGroup(),
              xskJob.getDescription(),
              xskJob.getModule(),
              ...
              xskJob.getParametersAsMap());
          logger.info(
              "Synchronized a modified Job [{}] from group: {}",
              xskJob.getName(),
              xskJob.getGroup());
        }
      }
