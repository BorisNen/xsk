...
public class XSKDataStructuresSynchronizer extends AbstractSynchronizer {
  ...
  private static final Map<String, XSKDataStructureEntitiesModel> ENTITIES_PREDELIVERED =
      Collections.synchronizedMap(new HashMap<>());
  private static final Map<String, XSKDataStructureHDBTableModel> TABLES_PREDELIVERED =
      Collections.synchronizedMap(new HashMap<>());
  private static final Map<String, XSKDataStructureHDBViewModel> VIEWS_PREDELIVERED =
      Collections.synchronizedMap(new HashMap<>());
  private static final Map<String, XSKDataStructureHDBProcedureModel> PROCEDURES_PREDELIVERED =
      Collections.synchronizedMap(new HashMap<>());
  private static final Map<String, XSKDataStructureHDBTableFunctionModel>
      TABLEFUNCTIONS_PREDELIVERED = Collections.synchronizedMap(new HashMap<>());
  private static final Map<String, XSKDataStructureHDBSchemaModel> SCHEMAS_PREDELIVERED =
      Collections.synchronizedMap(new HashMap<>());
  private static final Map<String, XSKDataStructureHDBSynonymModel> SYNONYMS_PREDELIVERED =
      Collections.synchronizedMap(new HashMap<>());
  ...
  public static final void forceSynchronization() {
    XSKDataStructuresSynchronizer dataStructureSynchronizer =
  ...
  public void registerPredeliveredEntities(String contentPath) throws Exception {
    ...
    XSKDataStructureEntitiesModel model =
        (XSKDataStructureEntitiesModel)
            xskCoreParserService.parseDataStructure(
                IXSKDataStructureModel.FILE_EXTENSION_ENTITIES, contentPath, data);
  ...
  public void registerPredeliveredTable(String contentPath) throws Exception {
    ...
    XSKDataStructureHDBTableModel model =
        (XSKDataStructureHDBTableModel)
            xskCoreParserService.parseDataStructure(
                IXSKDataStructureModel.FILE_EXTENSION_TABLE, contentPath, data);
  ...
  public void registerPredeliveredView(String contentPath) throws Exception {
    ...
    XSKDataStructureHDBViewModel model =
        (XSKDataStructureHDBViewModel)
            xskCoreParserService.parseDataStructure(
                IXSKDataStructureModel.FILE_EXTENSION_VIEW, contentPath, data);
  ...
  public void registerPredeliveredSynonym(String contentPath) throws Exception {
    ...
    XSKDataStructureHDBSynonymModel model =
        (XSKDataStructureHDBSynonymModel)
            xskCoreParserService.parseDataStructure(
                IXSKDataStructureModel.FILE_EXTENSION_SYNONYM, contentPath, data);
  ...
  public void registerPredeliveredHDBProcedure(String contentPath)
    ...
    model =
        (XSKDataStructureHDBProcedureModel)
            xskCoreParserService.parseDataStructure(
                IXSKDataStructureModel.FILE_EXTENSION_HDBPROCEDURE, contentPath, data);
  ...
  public void registerPredeliveredHDBTableFunction(String contentPath)
    ...
    model =
        (XSKDataStructureHDBTableFunctionModel)
            xskCoreParserService.parseDataStructure(
                IXSKDataStructureModel.FILE_EXTENSION_HDBTABLEFUNCTION, contentPath, data);
  ...
  public void registerPredeliveredHDBSchema(String contentPath)
    ...
    model =
        (XSKDataStructureHDBSchemaModel)
            xskCoreParserService.parseDataStructure(
                IXSKDataStructureModel.FILE_EXTENSION_HDBSCHEMA, contentPath, data);
    ...
    synchronized (XSKDataStructuresSynchronizer.class) {
      ...
      try {
        ...
        successfulSynchronization(
            SYNCHRONIZER_NAME,
            format(
                "Immutable: [Entities: {0}, Tables: {1}, Views: {2}, Procedures: {3}, Functions: {4}, Schemas: {5}, HDI: {6}], "
                    + "Mutable: [Entities: {7}, Tables: {8}, Views: {9}, Procedures: {10}, Functions: {11}, Schemas: {12}, HDI: {13}, Syninyms: {13}]",
                immutableEntitiesCount,
                immutableTablesCount,
                immutableViewsCount,
                immutableProceduresCount,
                immutableFunctionsCount,
                immutableSchemasCount));
        //						mutableEntitiesCount, mutableTablesCount, mutableViewsCount, mutableProceduresCount,
        // mutableFunctionsCount, mutableSchemasCount, mutableHDICount, immutableSynonymsCount));
      } catch (Exception e) {
        ...
        try {
          ...
          logger.error(
              "Synchronizing process for HDB Data Structures files failed in registering the state log.",
              e);
        }
      }
  ...
  private void synchronizePredelivered() throws SynchronizationException {
    ...
    for (XSKDataStructureHDBSchemaModel hdbSchema : SCHEMAS_PREDELIVERED.values()) {
      try {
        xskHDBCoreFacade.handleResourceSynchronization(
            IXSKDataStructureModel.FILE_EXTENSION_HDBSCHEMA, hdbSchema);
      } catch (Exception e) {
        logger.error(
            format(
                "Update hdbschema [{0}] skipped due to an error: {1}", hdbSchema, e.getMessage()),
            e);
      }
    }
    ...
    for (XSKDataStructureEntitiesModel entity : ENTITIES_PREDELIVERED.values()) {
      try {
        xskHDBCoreFacade.handleResourceSynchronization(
            IXSKDataStructureModel.FILE_EXTENSION_ENTITIES, entity);
      } catch (Exception e) {
        logger.error(
            format("Update entities [{0}] skipped due to an error: {1}", entity, e.getMessage()),
            e);
      }
    }
    ...
    for (XSKDataStructureHDBTableModel table : TABLES_PREDELIVERED.values()) {
      try {
        xskHDBCoreFacade.handleResourceSynchronization(
            IXSKDataStructureModel.FILE_EXTENSION_TABLE, table);
      } catch (Exception e) {
        logger.error(
            format("Update tables [{0}] skipped due to an error: {1}", table, e.getMessage()), e);
      }
    }
    ...
    for (XSKDataStructureHDBViewModel view : VIEWS_PREDELIVERED.values()) {
      try {
        xskHDBCoreFacade.handleResourceSynchronization(
            IXSKDataStructureModel.FILE_EXTENSION_VIEW, view);
      } catch (Exception e) {
        logger.error(
            format("Update views [{0}] skipped due to an error: {1}", view, e.getMessage()), e);
      }
    }
    ...
    for (XSKDataStructureHDBProcedureModel hdbProcedure : PROCEDURES_PREDELIVERED.values()) {
      try {
        xskHDBCoreFacade.handleResourceSynchronization(
            IXSKDataStructureModel.FILE_EXTENSION_HDBPROCEDURE, hdbProcedure);
      } catch (Exception e) {
        logger.error(
            format(
                "Update hdbprocedure [{0}] skipped due to an error: {1}",
                hdbProcedure, e.getMessage()),
            e);
      }
    }
    ...
    for (XSKDataStructureHDBTableFunctionModel hdbTableFunction :
        TABLEFUNCTIONS_PREDELIVERED.values()) {
      try {
        xskHDBCoreFacade.handleResourceSynchronization(
            IXSKDataStructureModel.FILE_EXTENSION_HDBTABLEFUNCTION, hdbTableFunction);
      } catch (Exception e) {
        logger.error(
            format(
                "Update hdbtablefunction [{0}] skipped due to an error: {1}",
                hdbTableFunction, e.getMessage()),
            e);
      }
    }
    ...
    for (XSKDataStructureHDBSynonymModel synonym : SYNONYMS_PREDELIVERED.values()) {
      try {
        xskHDBCoreFacade.handleResourceSynchronization(
            IXSKDataStructureModel.FILE_EXTENSION_SYNONYM, synonym);
      } catch (Exception e) {
        logger.error(
            format("Update synonyms [{0}] skipped due to an error: {1}", synonym, e.getMessage()),
            e);
      }
    }
