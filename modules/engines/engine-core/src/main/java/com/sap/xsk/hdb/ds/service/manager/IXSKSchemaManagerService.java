...
public class IXSKSchemaManagerService
    extends AbstractDataStructureManagerService<XSKDataStructureHDBSchemaModel> {
  ...
  @Override
  public void synchronizeRuntimeMetadata(XSKDataStructureHDBSchemaModel schemaModel)
    ...
    if (!getDataStructuresCoreService()
        .existsDataStructure(schemaModel.getLocation(), schemaModel.getType())) {
      ...
      logger.info(
          "Synchronized a new HDB Schema file [{}] from location: {}",
          schemaModel.getName(),
          schemaModel.getLocation());
    } else {
      XSKDataStructureHDBSchemaModel existing =
          getDataStructuresCoreService()
      ...
      if (!schemaModel.equals(existing)) {
        ...
        dataStructureSchemasModels.put(schemaModel.getName(), schemaModel);
        logger.info(
            "Synchronized a modified HDB Schema file [{}] from location: {}",
            schemaModel.getName(),
            schemaModel.getLocation());
      }
    }
