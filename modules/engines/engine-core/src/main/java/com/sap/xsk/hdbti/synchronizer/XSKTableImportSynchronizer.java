...
public class XSKTableImportSynchronizer extends AbstractSynchronizer {
  ...
  @Inject private XSKTableImportParser xskTableImportParser;

  @Inject private XSKHDBTIProcessor XSKHDBTIProcessor;

  @Inject private XSKCsvToHdbtiRelationDao xskCsvToHdbtiRelationDao;

  @Inject private XSKTableImportCoreService xskTableImportCoreService;

  @Inject
  private static final Logger logger = LoggerFactory.getLogger(XSKTableImportSynchronizer.class);

  private static final Map<String, XSKTableImportArtifact> HDBTI_PREDELIVERED =
      Collections.synchronizedMap(new HashMap<>()); // ones which already exist in the JAR

  private static final List<String> HDBTI_SYNCHRONIZED =
      Collections.synchronizedList(
          new ArrayList<>()); // used for leaving only the correct files after the sync

  private static final Map<String, XSKTableImportArtifact> HDBTI_MODELS =
      new LinkedHashMap<>(); // used for collecting all created/updated models and later for the
                             // actual execution of the query ( import/ alter etc)

  @Inject private DataSource dataSource;
  @Inject private XSKCSVRecordDao xskcsvRecordDao;
  @Inject private XSKImportedCSVRecordDao xskImportedCSVRecordDao;
        ...
        try {
          if (isSynchronizerSuccessful(
            ...
            successfulSynchronization(
                SYNCHRONIZER_NAME,
                format("Immutable: {0}, Mutable: {1}", immutableCount, mutableCount));
          } else {
          ...
          try {
            ...
            logger.error(
                "Synchronizing process for HDBTI files failed in registering the state log.", e);
          }
        }
  ...
  public static final void forceSynchronization() {
    XSKTableImportSynchronizer synchronizer =
        StaticInjector.getInjector().getInstance(XSKTableImportSynchronizer.class);
  ...
  protected void synchronizeResource(IResource resource) throws SynchronizationException {
    ...
    if (resourceName.endsWith(IXSKTableImportModel.FILE_EXTENSION_TABLE_IMPORT)) {
      try {
        XSKTableImportArtifact xskTableImportArtifact =
            xskTableImportParser.parseTableImportArtifact(registryPath, contentAsString);
      ...
      List<XSKTableImportToCsvRelation> affectedHdbtiToCsvRelations =
          xskCsvToHdbtiRelationDao.getAffectedHdbtiToCsvRelations(getRegistryPath(resource));
      if (!affectedHdbtiToCsvRelations.isEmpty()) {
        if (xskCsvToHdbtiRelationDao.hasCsvChanged(
            affectedHdbtiToCsvRelations.get(0), contentAsString)) {
          affectedHdbtiToCsvRelations.forEach(
              relation ->
                  reimportAffectedHdbtiFiles(XSKUtils.convertToFullPath(relation.getHdbti())));
        }
      }
    }
  }
  ...
  private void reimportAffectedHdbtiFiles(String hdbtiFilePath) {
    ...
    try (Connection connection = dataSource.getConnection()) {
      XSKTableImportArtifact xskTableImportArtifact =
          xskTableImportParser.parseTableImportArtifact(
              hdbtiFilePath, getContentFromResource(hdbtiResource));
      executeTableImport(xskTableImportArtifact, connection);
    } catch (IOException | SynchronizationException | SQLException e) {
      logger.error(
          "Error during the force reimport of an HDBTI file due to a linked csv file change", e);
    } catch (XSKHDBTISyntaxErrorException syntaxErrorException) {
  ...
  private void synchronizeTableImport(XSKTableImportArtifact xskTableImportArtifact)
      throws SynchronizationException {
    try {
      if (!xskTableImportCoreService.existsTableImportArtifact(
        ...
        logger.info(
            "Synchronized a new HDBTI file [{}] from location: {}",
            xskTableImportArtifact.getName(),
            xskTableImportArtifact.getLocation());
      } else {
        XSKDataStructureModel existing =
            xskTableImportCoreService.getTableImportArtifact(xskTableImportArtifact.getLocation());
        if (!xskTableImportArtifact.equals(existing)) {
          xskTableImportCoreService.updateTableImportArtifact(xskTableImportArtifact);
          HDBTI_MODELS.put(xskTableImportArtifact.getName(), xskTableImportArtifact);
          logger.info(
              "Synchronized a modified HDBTI file [{}] from location: {}",
              xskTableImportArtifact.getName(),
              xskTableImportArtifact.getLocation());
        }
      }
  ...
  protected void cleanup() throws SynchronizationException {
    try {
      List<XSKTableImportArtifact> tableImportArtifacts =
          xskTableImportCoreService.getTableImportArtifacts();

      IRepository repository = getRepository();
      for (XSKTableImportArtifact tableImportArtifact : tableImportArtifacts) {
        if (!repository.hasResource(
            XSKUtils.convertToFullPath(tableImportArtifact.getLocation()))) {
          xskTableImportCoreService.removeTableImportArtifact(tableImportArtifact.getLocation());
          xskCsvToHdbtiRelationDao.deleteCsvAndHdbtiRelations(
              tableImportArtifact.getLocation(), dataSource.getConnection());
          removeCSVRecordsFromDb(tableImportArtifact.getLocation());
          logger.warn(
              "Cleaned up HDBTI file [{}] from location: {}",
              tableImportArtifact.getName(),
  ...
  private void removeCSVRecordsFromDb(String hdbtiLocation) {
    List<XSKImportedCSVRecordModel> csvRecordsToRemove =
        xskImportedCSVRecordDao.getImportedCSVsByHdbtiLocation(hdbtiLocation);
    csvRecordsToRemove.sort(
        Comparator.comparing(XSKImportedCSVRecordModel::getCreatedAt, Comparator.reverseOrder()));
    try {
      for (XSKImportedCSVRecordModel csvRecord : csvRecordsToRemove) {
      ...
      logger.error(
          String.format(
              "Error occurred while trying to remove imported csv records after a deletion of an hdbti file with location: %s",
              hdbtiLocation),
          sqlException);
    }
  }
  ...
  private void executeTableImport(
      XSKTableImportArtifact tableImportArtifact, Connection connection) {
    List<XSKTableImportConfigurationDefinition> configurationDefinitions =
        tableImportArtifact.getImportConfigurationDefinition();

    xskCsvToHdbtiRelationDao.deleteCsvAndHdbtiRelations(
        XSKUtils.convertToFullPath(tableImportArtifact.getLocation()), connection);
  ...
  public void registerPredeliveredTableImports(String contentPath) throws Exception {
    ...
    XSKTableImportArtifact tableImportArtifact =
        xskTableImportParser.parseTableImportArtifact(contentPath, data);
