...
public class XSKHDBTICoreListener extends HdbtiBaseListener {
  ...
  public void exitObjConfig(HdbtiParser.ObjConfigContext ctx) {
    ...
    for (HdbtiParser.AssignExpressionContext expressionContext : ctx.assignExpression()) {
      ...
      } else if (expressionContext.getChild(0)
          instanceof HdbtiParser.AssignDistinguishEmptyFromNullContext) {
  ...
  public void enterAssignExpression(HdbtiParser.AssignExpressionContext ctx) {
    ...
    if (usedFields.contains(currentObjField)) {
      throw new DuplicateFieldNameException(
          String.format(
              "Duplicate fields: Field with name: '%s' is already in use.", currentObjField));
    } else {
  ...
  public void exitPair(HdbtiParser.PairContext ctx) {
    if (ctx.pairKey() != null && ctx.pairValue() != null) {
      XSKHDBTIImportConfigModel.Pair pair =
          new XSKHDBTIImportConfigModel.Pair(
              tokens.get(ctx.pairKey()), tokens.get(ctx.pairValue()));
      pairs.add(pair);
    }
  }
  ...
  @Override
  public void exitAssignDistinguishEmptyFromNull(
      HdbtiParser.AssignDistinguishEmptyFromNullContext ctx) {
    if (ctx != null && ctx.BOOLEAN() != null) {
      tokens.put(ctx, ctx.BOOLEAN().getText());
    }
  }
