...
public enum JoinType {
  ...
  /**
   * Join type referential integrity means for each row in the left table there is a corresponding
   * row in the right table. If this join type is selected the engine can better optimize the
   * queries, because the join can be completely ignored if no fields are requested from the joined
   * table.
   */
  ...
  /**
   * TextTable indicates a join with a text table (as right table) having language as additional
   * key. During runtime the language key is filled with the user/logon language
   */
