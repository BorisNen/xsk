...
public enum RootNodeVisibility {
   ...
   * have the following simple table with a parent child hierarchy defined on top: A->B and A->C
   * Then we have: ADD_ROOT_NODE_IF_DEFINED, ADD_ROOT_NODE: Results in a 2 level hierarchy with A at
   * the root having B, C as children DO_NOT_ADD_ROOT_NODE: Results in a 1 level hierarchy with B
   * and C as independent root nodes Example 2: Assume we have the following simple table with a
   * parent child hierarchy defined on top. null->A and A->B and A->C Then we have:
   * ADD_ROOT_NODE_IF_DEFINED, DO_NOT_ADD_ROOT_NODE: Results in a 2 level hierarchy with A at the
   * root having B,C as children ADD_ROOT_NODE: Results in a 3 level hierarchy with null as root
   * node having A as child and A having B,C as children.
   */
