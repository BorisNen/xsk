...
public class XSKHDBSYNONYMCoreListener extends HdbsynonymBaseListener {
  ...
  /**
   * Currently the Parser do not take into account if a given property is mandatory. If the property
   * order is misplaced the parser will still parse the values. If more than one value of one
   * property is provided then only the first one is taken.
   */
  @Override
  public void exitHdbsynonymDefinition(HdbsynonymParser.HdbsynonymDefinitionContext ctx) {
    ...
    if (ctx.synonymBody().synonymSchema() != null && ctx.synonymBody().synonymSchema().size() > 0) {
      model.setSynonymSchema(
          handleStringLiteral((String) tokens.get(ctx.synonymBody().synonymSchema().get(0))));
    }

    if (!(ctx.synonymBody().synonymTarget() == null
        || ctx.synonymBody().synonymTarget().isEmpty())) {
